        ...
        private void checkBox_Rotate_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (checkBox_Rotate.Checked == true)
            {
                ...
                pictureBox_Polygon.Image = buf;
                    ...
                    if (buf != null)
                    {
                        if (this.checkBox_Rotate.Checked == true)
                        {
                            buf.RotateFlip(RotateFlipType.Rotate270FlipY);//Rotate180FlipY);
                            pictureBox_Polygon.Image = buf;
                            Thread.Sleep((int)this.numericUpDown_Rotate.Value);
                            if (this.checkBox_Rotate.Checked == true)
                                checkBox_Rotate_CheckedChanged(sender, e);
                                //return;
                        //break;
                        //if (this.checkBox_Rotate.Checked == false)
                        //return;
                        //return;
                        }
                    }
